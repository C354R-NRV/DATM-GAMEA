...
function get_next_correlative($usuario_, $unidad, $codigo, $referencia, $usuario_solicitante, $destino_, $hhrr_)
{
    ...
    $conn = new Conexion();
    $cons = $conn->conectar();
    $valid_units = ['UAJ-CC', 'UF', 'UA', 'UICT', 'SIS', 'DIR'];
    /* $valid_codes = ['RA', 'PR', 'ITL', 'IE', 'AA', 'AEF', 'PIET', 'NE', 'IC', 'OFJ']; */
    $baseCite = '';
    if (!in_array(strtoupper($unidad), $valid_units)) {
        throw new Exception('Unidad no valida ->' . $unidad);
    }
